...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for(int t=0; t<30000; t++){
		...
		int forest_output[100];
		for(int i=0; i<100; i++){
			forest_output[i] = TreeError(data[i],tree[t]); // use origin data to get g(t)
		...
		for(int i=0; i<100; i++){
			if(forest_output[i]!=data[i][2]){
				error_per_t++;
			}
		}
		error_out[t] = error_per_t/(double)100;
